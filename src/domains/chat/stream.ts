...
async function* solarWithFallback(
    ...
    try {
        for await (const chunk of solarChatStream('', messages)) {
            if (chunk.text) {
                firstChunkSeen = true
    ...
    try {
        const gemini = await generateGeminiStream(systemPrompt, history)
...
async function* geminiOnly(systemPrompt: string, history: GeminiMessage[]): AsyncGenerator<TextChunk> {
    try {
        const gemini = await generateGeminiStream(systemPrompt, history)
        for await (const chunk of gemini) yield { text: chunk.text || '' }
    } catch (err) {
...
export async function generateChatStream(
    ...
    if (driver === 'solar') return solarWithFallback(systemPrompt, history)
    if (driver === 'gemini') return geminiOnly(systemPrompt, history)
