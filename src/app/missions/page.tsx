...
export default function MissionsPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (user) {
                ...
                // 3) 클로버 잔고 (users 테이블에 clovers 컬럼 — 없으면 0)
                ...
                setMissionStatus({
                    aiCreated: (mentorCount || 0) > 0,
                    questionsAsked: Math.min(msgCount || 0, 10),
                    friendsInvited: 0, // 추후 추적
                })
    ...
    const handleCopyInviteLink = () => {
        const link = `https://app-seven-delta-90.vercel.app/?ref=${user?.id?.slice(0, 8) || 'curi'}`
    ...
    const missions: MissionItem[] = [
        {
            ...
            icon: '🤖',
            title: '내 AI 만들어보기',
            description: '나만의 AI를 처음으로 만들어보세요',
            reward: 50,
            progress: missionStatus.aiCreated ? 1 : 0,
            goal: 1,
            completed: missionStatus.aiCreated,
        ...
        },
        {
            ...
            icon: '🎉',
            title: '친구 초대하기',
            description: '초대 링크를 공유해 친구를 초대하세요',
            reward: 100,
                        ...
                        <div style={{ animation: 'fadeIn 0.4s ease' }}>
                            ...
                            <div style={{
                                ...
                                <div>
                                    ...
                                    <div style={{
                                        fontSize: 28, fontWeight: 800, color: '#15803d',
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <input
                                readOnly
                                value={`https://app-seven-delta-90.vercel.app/?ref=${user?.id?.slice(0, 8) || 'curi'}`}
                                style={{
                        ...
                        <div style={{
                            ...
                            <button
                                onClick={() => {
                                    const link = `https://app-seven-delta-90.vercel.app/?ref=${user?.id?.slice(0, 8) || 'curi'}`
                                    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent('나만의 AI를 만들어보세요! 🤖 ' + link)}`, '_blank')
                                }}
                                style={{
                            ...
                            <button
                                onClick={() => {
                                    const link = `https://app-seven-delta-90.vercel.app/?ref=${user?.id?.slice(0, 8) || 'curi'}`
                                    window.open(`https://t.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent('나만의 AI를 만들어보세요! 🤖')}`, '_blank')
                                }}
                                style={{
                                    padding: '10px 20px', borderRadius: 10,
                                    border: '1px solid #e5e7eb', background: '#fff',
